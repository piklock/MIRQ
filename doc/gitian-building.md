...
Clone the git repositories for mrq and gitian and then checkout the mrq version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/mrq-crypto/mrq.git
cd mrq
...
Follow the instructions in [doc/release-process.md](release-process.md) in the mrq repository
...
gitian builds' in [doc/release-process.md](release-process.md) in the mrq repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/mrq/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/mrq-crypto/mrq
...
./bin/gbuild --commit mrq=${COMMIT} --url mrq=${URL} ../mrq/contrib/gitian-descriptors/gitian-osx.yml
```
...
and follow the steps in the build process as normal.
    ...
    gpg: skipped "mrqcoin": secret key not available

When you execute `gsign` you will get an error from GPG, which can be ignored. Copy the resulting `.assert` files
in `gitian.sigs` to your signing machine and do

```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/mrq-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/mrq-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/mrq-build.assert
```
...
[mrq/gitian.sigs](https://github.com/mrq-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
